...
To approximate $\theta = \int x f(x) dx$, we can obtain an iid random sample $X_1, \dots, X_n$ from $f$ and then approximate $\theta$ via the sample average
...
But, Monte Carlo integration is a **very** powerful tool. While numerical integration methods are difficult to extend to multiple dimensions and work best ith a smooth integrand, Monte Carlo does not suffer these weaknesses.
...
```{example}
Estimate $\theta = \int_0^1 g(x) dx$.
```

[ ]{.pagebreak}

```{example}
Estimate $\theta = \int_a^b g(x) dx$.
```
...
```{example}
Monte Carlo integration for the standard Normal cdf ($N(0, 1)$). Let $X \sim N(0, 1)$, then the pdf of $X$ is
...
So, we can construct confidence intervals for our estimates 
